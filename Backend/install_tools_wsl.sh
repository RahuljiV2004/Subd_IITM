...
if ! command_exists sqlmap; then
    if sudo pip3 install sqlmap >> "$LOG_FILE" 2>&1; then
...
if [[ -f "$TOOLS_DIR/nikto/program/nikto.pl" ]]; then
    sudo ln -sf "$TOOLS_DIR/nikto/program/nikto.pl" /usr/local/bin/nikto
    log_message "SUCCESS" "Nikto linked to /usr/local/bin/nikto"
fi

# Link testssl
if [[ -f "$TOOLS_DIR/testssl.sh/testssl.sh" ]]; then
    sudo ln -sf "$TOOLS_DIR/testssl.sh/testssl.sh" /usr/local/bin/testssl
...
sudo ln -sf "$TOOLS_DIR/nikto/program/nikto.pl" /usr/local/bin/nikto.pl
